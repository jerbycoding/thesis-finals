...
| **MALWARE-002**       | "High-priority heuristic alert on [b]{victim_host}[/b]. Our monitoring system has detected an encoded [color=#006CFF]POWERSHELL[/color] script attempting to bypass standard signatures using 'Polymorphic' obfuscation." | **SIEM** (Search `Polymorphic` for LOG-MAL-002-A + Search `185.12.44.11` for LOG-MAL-002-B) -> Identify Host -> **Terminal** (`isolate`). |
