...
## Description
Create a state-driven AI that "simulates" a SOC analyst, provides clear feedback to the player, and uses safe signal connection hygiene.

## Implementation Details
*   **Singleton:** [x] Create `RivalAI.gd`.
*   **State Machine:**
    *   **[BLOCKER]** [x] All state changes **must** go through a central `_transition_to(new_state)` method. Direct assignment (`current_state = new_state`) is forbidden. This method is responsible for emitting the `rival_ai_state_changed` signal.
    *   [x] Reads `TraceLevelManager.get_trace_level()` to trigger transitions based on thresholds (`IDLE` < 30, `SEARCHING` 30-70, `LOCKDOWN` > 70).
*   **Player Feedback:**
...
*   **Role Guard & Signal Hygiene:**
    ...
    *   [x] A standard `if GameState.current_role != Role.HACKER: return` guard should still be used in `_process` as a second layer of safety.
*   **Speed Scaling:** [ ] AI "decision" timers (like the isolation countdown) must be scaled by `HeatManager.heat_multiplier`.

## Success Criteria
- [x] **[BLOCKER]** A private `_transition_to()` method is the only function that changes `RivalAI.current_state`.
...
- [x] The `RivalAI` singleton is created and added to autoload.
