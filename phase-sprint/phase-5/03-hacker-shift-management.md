...
*   **Action:** When `GameState.current_role == Role.HACKER`, the `NarrativeDirector` must be modified to load shifts from the `res://resources/hacker_shifts/` directory instead of the default analyst shifts.
...
## Success Criteria
- [ ] `HackerShiftResource.gd` script is created.
- [ ] The game correctly loads hacker-specific shifts when playing as the hacker.
